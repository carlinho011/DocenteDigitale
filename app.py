...
# STILE GRAFICO APPLICATO ALL'INTERA APPLICAZIONE (Anteprima stile foglio A4)
st.markdown("""
    <style>
    .foglio-word {
...
if modalita == "🚀 Genera Nuova Verifica":
    ...
    with col1: argomento = st.text_input("Inserisci l'argomento della verifica:", placeholder="Es. I vulcani...")
    with col2: stile_domande = st.selectbox("Tipo di domande:", ["Domande miste", "Risposte aperte", "Scelta multipla", "Vero o Falso"])
    with col3: difficolta = st.selectbox("Livello di difficoltà:", ["facile", "media", "difficile"])
    ...
    if st.button("Genera Testo Verifica"):
        if not argomento:
            ...
            with st.spinner("Generazione compito in corso con Gemini..."):
                prompt_sistema = "Sei un assistente didattico esperto per i licei e gli istituti tecnici italiani (Scuola Superiore). Genera la verifica e le relative risposte esclusivamente in lingua italiana. Il livello di complessità generale deve essere calibrato per gli standard delle scuole superiori. Inserisci obbligatoriamente il tag specifico [SOLUZIONI] subito prima di iniziare a scrivere le chiavi di correzione."
                prompt_utente = "Crea una verifica superiore di livello " + difficolta + " su " + argomento + ". Tipo domande: " + stile_domande + ". Numero quesiti: " + str(numero_domande) + "."
                
                try:
                    risposta = client.models.generate_content(model='gemini-2.5-flash', contents=prompt_utente, config={'system_instruction': prompt_sistema, 'temperature': 0.6})
                    ...
                    if "429" in str(e) or "quota" in str(e).lower() or "exhausted" in str(e).lower():
                        ...
                        try:
                            risposta = client.models.generate_content(model='gemini-2.5-pro', contents=prompt_utente, config={'system_instruction': prompt_sistema, 'temperature': 0.6})
                            st.session_state["testo_verifica"] = risposta.text
                            st.success("Verifica generata su linea Pro!")
                        except Exception as e2:
                            st.warning("⚠️ Linea Pro carica. Tentativo finale su linea Flash-8B...")
                            try:
                                risposta = client.models.generate_content(model='gemini-2.5-flash-8b', contents=prompt_utente, config={'system_instruction': prompt_sistema, 'temperature': 0.6})
                                st.session_state["testo_verifica"] = risposta.text
                                st.success("Verifica generata su linea Flash-8B!")
                            except Exception as final_err: st.error(f"❌ Linee Google sature: {final_err}")
                    else: st.error(f"⚠️ Errore di generazione: {e}")

    if "testo_verifica" in st.session_state:
        testo_grezzo = st.session_state['testo_verifica']
        st.write("### 📄 Esporta Documento")
        slug_argomento = argomento.lower().replace(' ', '_')
        nome_file_pdf = "verifica_" + difficolta + "_" + slug_argomento + ".pdf"

        script_pdf_pulsante = "<div style='margin-bottom: 20px;'><button onclick='scaricaFilePDF()' style='background-color: #2e7d32; color: white; padding: 14px 28px; border: none; border-radius: 6px; cursor: pointer; font-size: 16px; font-weight: bold; box-shadow: 0 4px 6px rgba(0,0,0,0.15);'>📥 Scarica Verifica in PDF (Tasto Diretto)</button></div><script src='https://cloudflare.com'></script><script>function scaricaFilePDF() { var target = window.parent.document.getElementById('blocco-foglio-word-target'); if (!target) { alert('Attendi il caricamento.'); return; } var configurazione = { margin: 12, filename: '" + nome_file_pdf + "', image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true, letterRendering: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } }; html2pdf().set(configurazione).from(target).save(); }</script>"
        ...
        intestazione_word_html = "<table class='tabella-intestazione'><tr><td style='width: 60%; font-weight: bold;'>Istituto d'Istruzione Superiore</td><td style='width: 40%; text-align: right; font-weight: bold;'>Data: ____/____/________</td></tr><tr><td>Alunno/a: _____________________________________</td><td style='text-align: right;'>Classe: ____________  Sez. ____</td></tr><tr><td style='padding-top: 10px; font-size: 16px; font-weight: bold;'>Materia: Verifica scritta di approfondimento (" + difficolta.capitalize() + ")</td><td style='padding-top: 10px; text-align: right; font-size: 16px; font-weight: bold;'>Oggetto: " + argomento.capitalize() + "</td></tr></table>"
        st.markdown("<div id='blocco-foglio-word-target' class='foglio-word'>" + intestazione_word_html + corpo_documento_html + "</div>", unsafe_allow_html=True)

# ==========================================================
# SEZIONE 2: SCANSIONA E CORREGGI (COMPLETA E INCLUSA)
# ==========================================================
